...
Startpos = TseI;
Traj{1,1} = TscI*TceStandoff;
Traj{2,1}= TscI*TceGrasp;
Traj{3,1}= Traj{2,1}; %same postion, just close the gripper and wait
Traj{4,1}= Traj{1,1}; %back up to Pickup Standoff position, just with gripper closed
Traj{5,1}= TscF*TceStandoff;
Traj{6,1}= TscF*TceGrasp;
Traj{7,1}= Traj{6,1}; %same position just open gripper and wait
Traj{8,1}= Traj{5,1}; %back up to Final Standoff Position, with the gripper open
...
for n=1:8
    outputConfigs(n+1,:) = [Traj{n,1}(1,1),Traj{n,1}(1,2),Traj{n,1}(1,3),Traj{n,1}(2,1),Traj{n,1}(2,2),Traj{n,1}(2,3),Traj{n,1}(3,1),...
    Traj{n,1}(3,2),Traj{n,1}(3,3),Traj{n,1}(1,4),Traj{n,1}(2,4),Traj{n,1}(3,4),Gripperstate(n+1)];
end
